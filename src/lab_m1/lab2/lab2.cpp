...
void Lab2::Init()
{
    ...
    // Create a mesh box using custom data
    {
        vector<VertexFormat> vertices
        {
            ...
            VertexFormat(glm::vec3(0.0, 0.0, 1.0), glm::vec3(1.f, 1.f, 1.f)),
            VertexFormat(glm::vec3(1.0, 0.0, 1.0), glm::vec3(.2f, .2f, .2f)),
            VertexFormat(glm::vec3(0.0, 1.0, 1.0), glm::vec3(.3f, .3f, .3f)),
            VertexFormat(glm::vec3(1.0, 1.0, 1.0), glm::vec3(.4f, .4f, .4f)),
            VertexFormat(glm::vec3(0.0, 0.0, 0.0), glm::vec3(.5f, .5f, .5f)),
            VertexFormat(glm::vec3(1.0, 0.0, 0.0), glm::vec3(.6f, .6f, .6f)),
            VertexFormat(glm::vec3(0.0, 1.0, 0.0), glm::vec3(.7f, .7f, .7f)),
            VertexFormat(glm::vec3(1.0, 1.0, 0), glm::vec3(.134f, .7654f, .645f))
        };
    ...
    {
        vector<VertexFormat> vertices{ VertexFormat(glm::vec3(0.0, 0.0, 0.0), glm::vec3(.3f, .23f, .45f)) };
        vector<unsigned int> indices;

        float x_nou, y_nou;
        float x_original{ 0.0 }, y_original{ 2.0 };

        //int NUM_LAT{ 100 };
        float pi{ 3.14159 };
        float angle{ (float)(2 * pi / NUM_LAT) };
        float RADIUS{ 2.0 };
        float another_y;
        ...
        for (int i = 0; i <= NUM_LAT; i++) {
            ...
            x_nou = x_original + cos(angle * i) * RADIUS;
            y_nou = y_original + sin(angle * i) * RADIUS;

            cout << y_nou << endl;

            vertices.push_back(VertexFormat(glm::vec3(x_nou, y_nou, 0.0), glm::vec3(.3f, .23f, .45f)));
        ...
        // Actually create the mesh from the data
        CreateMesh("circle", vertices, indices, true);
    }
    ...
    // by definition!
    {
        ...
        {
            VertexFormat(glm::vec3(0.0, 0.0, 0.0), glm::vec3(.75f, .2f, .3f)),
            VertexFormat(glm::vec3(1.0, 0.0, 0.0), glm::vec3(1.0f, .23f, .56f)),
            VertexFormat(glm::vec3(0.5, 0.866, 0.0), glm::vec3(.1f, .7f, .675f)),
            VertexFormat(glm::vec3(0.5, 0.289, 0.816), glm::vec3(.1f, .3f, .4f))
        };
        ...
        meshes["tetrahedron"] = new Mesh("generated new form");
        meshes["tetrahedron"]->InitFromData(verticies, indices);

        CreateMesh("tetrahedron", verticies, indices);
    }
...

    {
        vector<VertexFormat> vertices
        {
            ...
            VertexFormat(glm::vec3(0, 1, 1), glm::vec3(0.5f, 0.5f, .5f)),
...
void Lab2::CreateMesh(const char *name, const std::vector<VertexFormat> &vertices, const std::vector<unsigned int> &indices, bool is_circle)
{
    GLuint VAO = 0;
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_A"], shaders["VertexColor"], glm::vec3(-1.5f, 2.5f, 0), glm::vec3(0.25f));

    // TODO(student): Draw the mesh that was created with `CreateMesh()`
    RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(0, 2.5f, 0), glm::vec3(0.25f));

    // TODO(student): Draw the tetrahedron
    RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(0, 0, 0), glm::vec3(3.f));

    // TODO(student): Draw the square
    RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(1, -3, 0), glm::vec3(0.25f));
