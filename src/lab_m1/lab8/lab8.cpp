...
{
    {
        ...
        RenderSimpleMesh(meshes["teapot"], shaders["LabShader"], modelMatrix, glm::vec3(0.8265, 0.018, 0.9));
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.2412, 0.91, 0.091));
    ...
    // Render ground
    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.92, 0.8731, 0.4508));
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        if (window->KeyHold(GLFW_KEY_W)) {
            ...
            lightPosition2 -= forward * deltaTime * speed;
        }
        if (window->KeyHold(GLFW_KEY_A)) {
            lightPosition -= right * deltaTime * speed;
            lightPosition2 -= right * deltaTime * speed;
        }
        ...
        if (window->KeyHold(GLFW_KEY_E)) {
            ...
            lightPosition2 += up * deltaTime * speed;
        }
       ...
       // cout << angleOY << endl;
        rot = glm::rotate(rot, angleOY, glm::vec3(0, 1, 0));
        rot = glm::rotate(rot, angleOX, glm::vec3(1, 0, 0));
